...
<div class="container-xxl flex-grow-1 container-p-y">
  ...
  <form method="POST" action="{{ route('keterlambatan.update', $keterlambatan->id) }}">
    @csrf
    @method('PUT')
    <div class="card">
      <div class="card-header">
        <h5>Edit Data Keterlambatan</h5>
      </div>
      <div class="card-body">
        <div class="row">
            ...
            <!-- Input untuk memilih kelas -->
              <div class="col-md-6">
                ...
                <select id="classroom_id" name="classroom_id" class="form-select" required>
                  ...
                  <option value="{{ $item->id }}" {{ $item->id == $keterlambatan->classroom_id ? 'selected' : '' }}>{{ $item->nama_kelas }}</option>
                  @endforeach
                </select>
              </div>
              ...
              <div class="col-md-6">
                ...
                <input type="text" id="siswa_nama" class="form-control" placeholder="Cari Nama Siswa" value="{{ $keterlambatan->siswa->nama_lengkap }}" required>
                <div id="autocomplete-suggestions" class="typeahead" type="text" autocomplete="off"></div>
                <input type="hidden" id="siswa_id" name="siswa_id" value="{{ $keterlambatan->siswa_id }}">
              </div>
              <!-- Input untuk tanggal keterlambatan -->
              <div class="col-md-6">
                <label for="tanggal_keterlambatan" class="form-label">Tanggal Keterlambatan</label>
                <input type="date" id="tanggal_keterlambatan" name="tanggal_keterlambatan" class="form-control" value="{{ $keterlambatan->tanggal_keterlambatan }}" required>
              </div>
              <!-- Input untuk waktu keterlambatan -->
              <div class="col-md-6">
                <label for="waktu_keterlambatan" class="form-label">Waktu Keterlambatan</label>
                <input type="time" id="waktu_keterlambatan" name="waktu_keterlambatan" class="form-control" value="{{ $keterlambatan->waktu_keterlambatan }}" required>
              </div>
            </div>
          </div>
        </div>
